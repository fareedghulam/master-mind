...
export default function RegistrationForm({ onRegister, onLoginWithCredentials, onGoogleLogin }: RegistrationFormProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (res.success) {
        setSuccess('گوگل لاگ ان کامیاب ہو گیا ہے!');
      } else {
        setError(res.error || 'گوگل سائن ان میں کوئی مسئلہ پیش آیا۔');
      }
    } catch (err: any) {
      setError(err?.message || 'گوگل لاگ ان میں خرابی پیش آئی۔');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {onGoogleLogin && (
            <div className="mb-6">
              <button
                ...
                disabled={isLoading}
                className="w-full bg-white hover:bg-slate-50 border-2 border-slate-200 text-slate-800 font-bold py-3.5 px-4 rounded-xl shadow-sm transition-all flex items-center justify-center gap-3 cursor-pointer hover:border-slate-300 disabled:opacity-50"
              >
                ...
                <span className="text-sm font-sans">گوگل کے ساتھ سائن ان کریں (Sign in with Google)</span>
              </button>
